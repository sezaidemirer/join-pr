...
export async function generateMetadata({ params }: NewsLayoutProps): Promise<Metadata> {
  ...
  let adminNews: Awaited<ReturnType<typeof getPublishedNewsBySlug>> = null;
  try {
    adminNews = await getPublishedNewsBySlug(slug);
  } catch {
    adminNews = null;
  }

  // Kullanıcının diline göre öncelik ver
  const current = adminNews
    ? {
        title: adminNews.title,
        category: adminNews.category || 'Haber',
  ...
  if (!current) {
    const notFoundTitle = locale === 'tr' ? 'Haber Bulunamadı | Join PR' : 'News Not Found | Join PR';
  ...
  const title = `${current.title} | Join PR`;
  const description = current.description || (locale === 'tr' 
    ? `${current.title} hakkında Join PR'da okuyun.`
    : `Read about ${current.title} on Join PR.`);
  ...
  return {
    ...
    twitter: {
      ...
      title,
      description,
      images: current.image ? [`${baseUrl}${current.image}`] : [`${baseUrl}/og-news.webp`],
    },
    alternates: {
      canonical: pageUrl,
      languages: {
