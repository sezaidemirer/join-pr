...
export default function NewsDetailPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`/api/news?slug=${encodeURIComponent(lookupSlug)}`);
        const data = await res.json();
        if (!res.ok) return;
        if (alive) setAdminNews(data.item || null);
      } catch {
  ...
  useEffect(() => {
    ...
    if (rawSlug && redirectSlug && rawSlug !== redirectSlug) {
  ...
  if (adminNews?.platform_links?.length) {
    externalLinks.push(...adminNews.platform_links);
        ...
        <div className="space-y-3 rounded-2xl border border-white/10 bg-white/5 p-4">
          ...
          <div className="grid grid-cols-3 gap-3 sm:grid-cols-4 md:grid-cols-6">
            {externalLinks.map((link) => (
              ...
              >
                                <div className="flex flex-1 items-center justify-center w-full min-h-[2.5rem]">
                  <Image
