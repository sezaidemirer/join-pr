...
// Ürdün Destinasyon Tanıtımı.pdf — Jordan Tourism Board, Royal Jordanian, Marriott
const MARRIOTT_KPIS = {
  posts: 11,
  influencers: 3,
  engagement: '1.13M',
...
const MARRIOTT_PERFORMANCE = {
  ...
  likes: '1.13M',
  comments: '5.19K',
  dateRange: '5 Mar 2023 – 17 Mar 2023',
};

/** PDF profil blokları; post sayıları 5+4+2=11 ile özet satırıyla uyumlu. */
const MARRIOTT_CREATORS = [
  {
    name: 'Gizem Güneş',
    handle: 'gizemgunes',
  ...
  },
  {
    name: 'Emin Günenç',
    handle: 'emingunenc',
    image: '/rixos-infleuncers/emin_gunenc.webp',
    posts: 2,
    views: '2.03M',
...
const MARRIOTT_ALL_INFLUENCERS: ParticipatingInfluencer[] = [
...
export default function MarriottDeadSeaPage() {
      ...
      <div className="mx-auto max-w-6xl px-6 py-12 pb-20">
        <div className="mb-8 flex flex-col items-center gap-4 text-center">
          ...
          <p className="max-w-2xl text-sm text-zinc-400 md:text-base">
            Jordan Tourism Board, Royal Jordanian ve Marriott iş birliğiyle yürütülen destinasyon iletişimi; Amman, Lut Gölü ve Petra hattını öne çıkaran içeriklerle raporlandı.
          </p>
        </div>
        ...
        <div className="mb-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="rounded-2xl border border-white/10 bg-zinc-900/60 px-6 py-5">
            ...
            <p className="mt-1 text-3xl font-bold text-sky-400">{MARRIOTT_KPIS.engagement}</p>
          </div>
          <div className="rounded-2xl border border-white/10 bg-zinc-900/60 px-6 py-5">
            <p className="text-xs font-medium uppercase tracking-wider text-zinc-400">Katma Değer (EMV)</p>
            <p className="mt-1 text-3xl font-bold text-emerald-400">{MARRIOTT_KPIS.value}</p>
          </div>
        </div>
            ...
            <div>
              ...
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <div className="rounded-xl border border-white/10 bg-zinc-900/40 p-4">
                  ...
                  <p className="mt-1 text-xl font-bold text-sky-400">{MARRIOTT_PERFORMANCE.views}</p>
                </div>
                <div className="rounded-xl border border-white/10 bg-zinc-900/40 p-4">
                  <p className="text-xs text-zinc-400">Engagements</p>
                  <p className="mt-1 text-xl font-bold text-white">{MARRIOTT_PERFORMANCE.engagements}</p>
                </div>
                <div className="rounded-xl border border-white/10 bg-zinc-900/40 p-4">
                  <p className="text-xs text-zinc-400">Engagement Rate</p>
                  <p className="mt-1 text-xl font-bold text-amber-400">{MARRIOTT_PERFORMANCE.engagementRate}</p>
                </div>
                <div className="rounded-xl border border-white/10 bg-zinc-900/40 p-4">
                  <p className="text-xs text-zinc-400">Reach</p>
                  <p className="mt-1 text-xl font-bold text-white">{MARRIOTT_PERFORMANCE.reach}</p>
                </div>
                <div className="rounded-xl border border-white/10 bg-zinc-900/40 p-4">
                  <p className="text-xs text-zinc-400">EMV</p>
                  <p className="mt-1 text-xl font-bold text-emerald-400">{MARRIOTT_PERFORMANCE.emv}</p>
                </div>
                <div className="rounded-xl border border-white/10 bg-zinc-900/40 p-4">
                  <p className="text-xs text-zinc-400">Followers (kitlenin toplamı)</p>
                  <p className="mt-1 text-xl font-bold text-white">{MARRIOTT_PERFORMANCE.followers}</p>
                </div>
                <div className="rounded-xl border border-white/10 bg-zinc-900/40 p-4">
                  <p className="text-xs text-zinc-400">Shares</p>
                  <p className="mt-1 text-xl font-bold text-white">{MARRIOTT_PERFORMANCE.shares}</p>
                </div>
                <div className="rounded-xl border border-white/10 bg-zinc-900/40 p-4">
                  <p className="text-xs text-zinc-400">Likes</p>
                  <p className="mt-1 text-xl font-bold text-white">{MARRIOTT_PERFORMANCE.likes}</p>
                </div>
                <div className="rounded-xl border border-white/10 bg-zinc-900/40 p-4">
                  <p className="text-xs text-zinc-400">Comments</p>
                  <p className="mt-1 text-xl font-bold text-white">{MARRIOTT_PERFORMANCE.comments}</p>
                </div>
                <div className="rounded-xl border border-white/10 bg-zinc-900/40 p-4">
                  <p className="text-xs text-zinc-400">Tarih Aralığı</p>
                  <p className="mt-1 text-sm font-medium text-white">{MARRIOTT_PERFORMANCE.dateRange}</p>
                </div>
              </div>
            </div>
