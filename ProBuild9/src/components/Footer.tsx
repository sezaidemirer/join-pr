...
const SOCIAL_LINKS = [
  { name: 'LinkedIn', href: 'https://www.linkedin.com/company/join-pr/', icon: LinkedInIcon },
  { name: 'Instagram', href: 'https://www.instagram.com/join.pr/', icon: InstagramIcon },
  { name: 'YouTube', href: 'https://www.youtube.com/@JoinContentsTV', icon: YoutubeIcon },
  { name: 'Facebook', href: 'https://www.facebook.com/joinproffical', icon: FacebookIcon },
] as const;
...
export function Footer() {
  ...
  const onSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok) {
        setFeedback({ type: 'error', message: data.error || footer.newsletterError });
        setIsSubmitting(false);
        return;
      }

      setEmail('');
      setFeedback({ type: 'success', message: footer.newsletterSuccess });
      setTimeout(() => setFeedback(null), 5000);
    } catch {
      setFeedback({ type: 'error', message: footer.newsletterError });
    } finally {
      setIsSubmitting(false);
    }
  };
    ...
    <footer className="relative mt-24 border-t border-white/5 bg-zinc-950/80">
      ...
      <div className="mx-auto max-w-7xl px-6 py-16 lg:px-10">
            ...
            <form onSubmit={onSubmit} className="mt-5 space-y-3">
              <div className="relative">
                <input
                  ...
                  className={`w-full rounded-xl border border-white/10 bg-black/40 px-4 py-3 pr-24 text-white placeholder:text-zinc-500 focus:border-sky-500 focus:outline-none focus:ring-2 focus:ring-sky-500/40 ${locale === 'en' ? 'text-xs' : 'text-sm'}`}
                  required
                />
                <button
                  type="submit"
                  disabled={isSubmitting}
                  className="absolute right-2 top-1/2 -translate-y-1/2 rounded-lg bg-gradient-to-r from-teal-500 via-sky-500 to-blue-600 px-3 py-1.5 text-sm font-semibold text-white shadow-lg shadow-sky-500/30 transition-transform hover:-translate-y-1 disabled:opacity-70 disabled:cursor-not-allowed disabled:hover:translate-y-[-50%]"
                >
                  {isSubmitting ? '...' : footer.newsletterCta}
                </button>
              </div>
              {feedback && (
                <p className={`text-xs font-medium ${feedback.type === 'success' ? 'text-teal-400' : 'text-red-400'}`}>
        ...
        <div className="mt-12 flex flex-col justify-between gap-4 border-t border-white/5 pt-6 text-xs text-zinc-500 sm:flex-row">
          ...
          <span>Join PR | İstanbul / Türkiye</span>
        </div>
      </div>
    </footer>
  );
