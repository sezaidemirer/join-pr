...
const BACKGROUND_MAP: Record<string, string> = {
  joinPr: `${BASE_PATH}/banner1.webp`,
  joinCreative: `${BASE_PATH}/banner2.webp`,
  joinSocial: `${BASE_PATH}/banner3_social_mediya.webp`,
  joinAds: `${BASE_PATH}/banner4_ads.webp`,
  joinLabAi: `${BASE_PATH}/banner5_ai_lab.webp`,
  joinEscapes: `${BASE_PATH}/banner6_joinescapes.webp`,
};

const MOBILE_BACKGROUND_MAP: Record<string, string> = {
  joinPr: `${BASE_PATH}/mobile_banner_pr2.webp`,
  joinCreative: `${BASE_PATH}/mobile_banner_creative2.webp`,
  joinSocial: `${BASE_PATH}/mobile_banner_social2.webp`,
  joinAds: `${BASE_PATH}/mobile_banner_ads2.webp`,
  joinLabAi: `${BASE_PATH}/mobile_banner_ai2.webp`,
  joinEscapes: `${BASE_PATH}/mobile_banner_escapes2.webp`,
};
...
export function HeroSlider() {
  ...
  const isFirstSlide = (index: number) => index === 0;
  const FIRST_SLIDE_KEY = slides[0]?.[0] || 'joinPr';
  const firstSlideDesktop = BACKGROUND_MAP[FIRST_SLIDE_KEY];
  const firstSlideMobile = MOBILE_BACKGROUND_MAP[FIRST_SLIDE_KEY];

  return (
    <section className="relative mt-6 overflow-hidden rounded-3xl bg-zinc-950 shadow-2xl shadow-black/40 lg:mt-8" data-hero>
      <div 
        className="relative min-h-[500px] sm:min-h-[520px] md:min-h-[560px] lg:min-h-[620px]"
      ...
      >
        {slides.map(([key, slide], index) => {
          ...
          const isFirst = isFirstSlide(index);
          const mobileImage = MOBILE_BACKGROUND_MAP[key];
          const desktopImage = BACKGROUND_MAP[key];
          const hasSeparateMobile = mobileImage && mobileImage !== desktopImage;
          ...
          return (
            ...
            >
              {/* Background image - LCP optimization: first slide uses <img>, others use CSS background */}
              {isFirst && hasSeparateMobile ? (
                <>
                  {/* First slide: Mobile image - eager loading for LCP */}
                  <img
                    src={firstSlideMobile}
                    alt=""
                    className="absolute inset-0 -z-10 h-full w-full object-contain object-center lg:hidden"
                    loading="eager"
                    fetchPriority="high"
                    width={750}
                    height={1000}
                    aria-hidden="true"
                  />
                  {/* First slide: Desktop image - eager loading for LCP */}
                  <img
                    src={firstSlideDesktop}
                    alt=""
                    className="absolute inset-0 -z-10 hidden h-full w-full object-contain object-center lg:block"
                    loading="eager"
                    fetchPriority="high"
                    width={3242}
                    height={1350}
                ...
                <img
                  src={firstSlideDesktop || DEFAULT_HERO_BG}
                  alt=""
                ...
                <>
                  {/* Non-first slides: Lazy loading with CSS background */}
                  <div
                    className="absolute inset-0 -z-10 bg-contain bg-center bg-no-repeat lg:hidden"
                    style={{ backgroundImage: `url(${mobileImage})` }}
                  />
                  <div
                    className="absolute inset-0 -z-10 hidden bg-contain bg-center bg-no-repeat lg:block"
                ...
                <div
                  ...
                  style={{ backgroundImage: `url(${desktopImage ?? DEFAULT_HERO_BG})` }}
                />
              )}
              
              {/* Gradient overlay - metinlerin okunabilirliği için */}
              <div className="absolute inset-0 -z-10 bg-gradient-to-b from-zinc-950/40 via-zinc-950/30 to-transparent lg:bg-gradient-to-r lg:from-zinc-950/50 lg:via-zinc-950/40 lg:to-transparent" />
              
              {/* Metin içeriği - görsel üzerine üstte */}
              <div className="flex flex-1 flex-col justify-start gap-3 sm:gap-4 md:gap-6 lg:justify-center lg:gap-8 pt-4 sm:pt-6 md:pt-8 lg:pt-4 lg:max-w-2xl z-10">
              ...
              <button
                ...
                className="absolute left-2 top-[60%] -translate-y-1/2 z-20 flex h-12 w-12 items-center justify-center rounded-full bg-white/20 backdrop-blur-sm border border-white/30 text-white transition-all hover:bg-white/30 hover:scale-110 active:scale-95 lg:hidden text-2xl font-bold"
              ...
              <button
                ...
                className="absolute right-2 top-[60%] -translate-y-1/2 z-20 flex h-12 w-12 items-center justify-center rounded-full bg-white/20 backdrop-blur-sm border border-white/30 text-white transition-all hover:bg-white/30 hover:scale-110 active:scale-95 lg:hidden text-2xl font-bold"
