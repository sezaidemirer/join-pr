...
function pressHrefFromTitle(title: string) {
  const slug = slugify(title);
  // Hurghada/1 basın yansıması: kullanıcı özellikle bu varyant slug'a gitsin istiyor.
  if (slug === 'turk-oyuncular-misirin-en-unlu-tatil-merkezinde-bulustu') {
    return 'https://joinpr.com.tr/haber/turk-oyuncular-misir-in-en-unlu-tatil-merkezinde-bulustu/';
  }
  return `/haber/${slug}`;
}
        ...
        <div className="rounded-3xl border border-white/10 bg-zinc-950/70 p-8 shadow-xl">
          ...
          <div className="space-y-4">
            {pressArticles.map((article: { title: string; category: string; description: string; image?: string }, idx: number) => {
              const href = pressHrefFromTitle(article.title);
              return (
                <Link
                key={idx}
                  href={href}
