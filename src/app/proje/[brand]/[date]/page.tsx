...
/** Next `output: export` bos `generateStaticParams` kabul etmez; DB bos/ag yoksa tek sahte path + notFound. */
const CPANEL_EXPORT_EMPTY_BRAND = '__cpanel-empty__';
const CPANEL_EXPORT_EMPTY_DATE = '0';

function isCpanelExportEmptySlug(brand: string, date: string) {
  return (
    process.env.STATIC_EXPORT === '1' &&
    brand === CPANEL_EXPORT_EMPTY_BRAND &&
    date === CPANEL_EXPORT_EMPTY_DATE
  );
}
...
export async function generateStaticParams() {
  ...
  try {
    const { listOfferPathParams } = await import('@/lib/offers');
    const paths = await listOfferPathParams();
    if (paths.length > 0) return paths;
  } catch {
    /* Supabase yok veya ag kesik */
  }
  return [{ brand: CPANEL_EXPORT_EMPTY_BRAND, date: CPANEL_EXPORT_EMPTY_DATE }];
}

export async function generateMetadata({ params }: PageParams): Promise<Metadata> {
  if (isCpanelExportEmptySlug(params.brand, params.date)) {
    notFound();
  }
