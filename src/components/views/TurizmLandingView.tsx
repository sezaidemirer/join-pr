...
const PROBLEMS = [
  ...
  { text: 'Sezon planlaması olmadan reklam çıkılması', impact: 82, icon: 'calendar' },
...
const SOLUTION_ITEMS = [
  {
    title: 'Destinasyon Bazlı Arama Hakimiyeti',
    mainDesc: 'Rakiplerinizin hangi destinasyonlardan rezervasyon aldığını ve hangi aramalarda öne çıktığını net bir şekilde görebilirsiniz.',
    keywords: ['Balkan turu fiyat', 'Kapadokya turu 2026', 'Mısır turu her şey dahil'],
    closing: 'Rezervasyona dönüşen aramalarda görünmeyen markalar pazarı rakiplerine kaptırır.',
  },
  {
    ...
    points: [
      ...
      'Sezonluk bütçe değişim analizi',
    ],
    closing: 'Kimin ne kadar harcadığını bilmeden doğru strateji kurulamaz.',
  },
  {
    title: 'Rezervasyon Odaklı Funnel Sistemi',
    mainDesc: 'Sadece tıklama değil, rezervasyon odaklı bir sistem.',
    points: [
      'WhatsApp üzerinden dönüşüm takibi',
      'Offline satış yükleme',
      'Akıllı retargeting',
      'Son dakika kampanya otomasyonu',
    ],
    closing: 'Reklamlardan gelen her talep satışa dönüştürülür.',
  },
];

const KPIS = [
  'Rezervasyon başı kârlılık',
  'Destinasyon bazlı talep dönüşümü',
  'Marka arama büyüme oranı',
  'Mutlak üst sıra hakimiyeti',
  'Lead → Satış kapanış oranı',
];
...
export function TurizmLandingView() {
  ...
  const onChange = (k: keyof FormState) => (e: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
  ...
  const onSubmit = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    setFeedback(null);
    const isComplete = form.company?.trim() && form.website?.trim() && form.phone?.trim() && form.email?.trim();
    if (!isComplete) {
      setFeedback('Lütfen formu eksiksiz doldurun.');
      return;
    }
    setIsSubmitting(true);
    const message = `Web sitesi: ${form.website || '-'}\nİletişim: ${form.phone || '-'}`;

    try {
      // Production: sendmail.php (static hosting). Dev: /api/contact
      ...
      const res = await fetch(apiUrl, {
        ...
        body: JSON.stringify({
          ...
          email: form.email,
          phone: form.phone,
          topic: 'Turizm - Ücretsiz Dijital Konum Analizi',
          message,
        }),
      });
      const data = await res.json();
      if (!res.ok) {
        setFeedback(data.error || 'Bir hata oluştu.');
    ...
    <div className="flex flex-col gap-0">
      ...
      <section className="relative left-1/2 right-1/2 -ml-[50vw] -mr-[50vw] w-screen overflow-hidden rounded-b-3xl border-b border-white/10 bg-gradient-to-br from-sky-950/70 via-blue-950/50 to-slate-900/80 py-20 md:py-28">
        ...
        <div className="relative mx-auto max-w-6xl px-6 text-center">
          <h1 className="text-3xl font-bold leading-tight text-white md:text-4xl lg:text-5xl xl:text-[2.75rem]">
            Rakipleriniz Hangi Destinasyonlarda Rezervasyon Topluyor Biliyor musunuz?
          </h1>
      ...
      <section className="relative mt-16 overflow-hidden rounded-3xl border border-sky-500/25 bg-gradient-to-br from-sky-950/30 via-zinc-950/90 to-zinc-950 p-8 shadow-2xl shadow-sky-950/20 md:p-12 lg:p-16 xl:p-20">
        ...
        <div className="mb-12 flex flex-col items-center gap-4 text-center lg:mb-16">
          <h2 className="text-3xl font-bold text-white md:text-4xl lg:text-5xl xl:text-[2.75rem]">
            ...
            Turizm Firmalarının %90'ı Bu 5 Hatayı Yapıyor.
          </h2>
      ...
      <section className="mt-16 space-y-10">
        <div className="text-center">
          ...
          <p className="mx-auto mt-3 max-w-2xl text-lg text-white md:text-xl">Turizm markaları için dijital talep, rekabet ve performans verilerini tek bir analiz modelinde birleştiriyoruz.</p>
        </div>
        ...
        <div className="grid gap-8 lg:grid-cols-[1fr_auto] lg:items-center">
          <div className="relative">
            ...
            {[
              { num: 1, title: 'Performans Dashboard', punch: 'Bütçe yönü netleşir.', accordion: ['Harcanan bütçe, rezervasyon ve kârlılık birlikte değerlendirilir.', 'ROAS trendi (haftalık & ortalama)', 'Rezervasyon başı maliyet', 'Kaçış oranı / verimsiz harcama', 'Ciro – Harcama dengesi'] },
              ...
              { num: 3, title: 'Aylık Turizm İstihbarat', punch: 'Doğru sezon, doğru destinasyon belirlenir.', accordion: ['Talep nerede yoğunlaşıyor, hangi ay hangi destinasyon öne çıkıyor analiz edilir.', 'Arama hacimleri ve trend değişimleri ölçülür', 'Sezonluk pik ve düşüşler belirlenir', 'İç / dış pazar talep ayrımı yapılır'] },
          ...
          <div className="relative flex justify-center lg:justify-end">
            <div className="relative">
              <Image src="/turizm-karakter.webp" alt="Join PR Turizm" width={400} height={520} className="relative z-10 h-auto w-80 object-contain md:w-96 lg:w-[420px]" priority />
        ...
        <div className="mt-8 rounded-2xl border border-emerald-500/25 bg-emerald-950/25 p-6 md:p-8">
          ...
          <p className="mt-4 text-center text-sm italic text-zinc-400">İstanbul merkezli tur operatörü · 2024 Q3 analizi</p>
        </div>

        <div className="mx-auto mt-10 max-w-4xl space-y-5 text-justify text-lg leading-relaxed text-white md:text-xl">
          <p>Turizm sektöründe sürdürülebilir büyüme, yalnızca reklam bütçesi harcamakla değil; talep analizi, rakip görünürlüğü ve performans metriklerinin birlikte yönetilmesiyle sağlanır.</p>
          <p>Join PR sistemi, arama hacimleri ve sezon trendlerinden Google Ads rekabet verilerine, ROAS ve rezervasyon başı maliyete kadar tüm kritik verileri tek bir kontrol panelinde birleştirerek turizm markalarının dijital konumunu net şekilde ortaya koyar ve bütçeyi kârlı büyüme için optimize eder.</p>
        </div>
      </section>
      ...
      <section className="mt-16 grid gap-10 rounded-3xl border border-sky-500/30 bg-gradient-to-br from-sky-950/40 via-blue-950/30 to-zinc-950 p-8 md:grid-cols-[1fr_1fr] md:p-12">
        <div className="flex flex-col justify-center space-y-6">
          <h2 className="text-3xl font-bold text-white md:text-4xl">
            2026 Sezonunda Rakiplerinizin Gerisinde Kalmayın.
          </h2>
          <p className="text-sm text-white">
            AI destekli rekabet raporu ile rakip bütçelerini, Google görünürlüğünü ve rezervasyon potansiyelinizi öğrenin.
          </p>
          <button
            type="button"
            onClick={() => {
  ...
  return (
    <div className="group relative flex min-h-[320px] flex-col overflow-hidden rounded-3xl border border-sky-500/20 bg-gradient-to-br from-zinc-900/95 via-sky-950/30 to-zinc-950 p-6 shadow-2xl shadow-sky-950/30 transition-all duration-500 hover:border-sky-400/40 hover:shadow-sky-500/20">
      ...
      <div className="flex items-center justify-between pb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-zinc-500">Rezervasyon & ROAS</p>
          </div>
        </div>
      ...
      <div className="mt-4 grid grid-cols-3 gap-2 border-t border-white/5 pt-4 text-center">
        ...
        <div>
          <p className="text-[10px] uppercase tracking-wider text-zinc-500">Rezervasyon</p>
...
function AIRaporMockup() {
  const metrics = [
    ...
    { label: 'Risk & Fırsat', value: 'Döviz riski / Balkan fırsatı', color: 'text-amber-200' },
  ];
  return (
